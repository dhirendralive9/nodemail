  ...
  for (const recipientAddr of allRecipients) {
    ...
    if (mailbox.forwardEnabled && mailbox.forwardTo && mailbox.forwardTo.length > 0) {
      try {
        ...
        if (fwdAddresses.length > 0) {
          await sendMail({
            from:    `"${emailData.fromName || emailData.from}" <${mailbox.address}>`,
            to:      fwdAddresses.join(", "),
            ...
            html:    `<div style="font-family:sans-serif;font-size:13px;color:#888;border-left:3px solid #ccc;padding-left:12px;margin-bottom:16px;">
                        ...
                        <strong>Original From:</strong> ${emailData.from}<br>
                        <strong>Date:</strong> ${emailData.date}<br>
                        <strong>Subject:</strong> ${emailData.subject}
                      </div>
                      ${emailData.htmlBody || emailData.textBody.replace(/\n/g, "<br>")}`,
            inReplyTo:  emailData.messageId || undefined,
            references: emailData.messageId ? [emailData.messageId] : undefined,
          });
          console.log(`  Forwarded to ${fwdAddresses.join(", ")}`);
        }
      } catch (fwdErr) {
